...
module.exports.handleReply = async function ({ api, event, handleReply }) {
    ...
    try {
        ...
        if (fs.statSync(path).size > 26214400) return api.sendMessage('😑aby oye yo bot 259mb ko file lai generated garxa hai yeti lamo song send garera bihe ma lagera jane ho 😂💝.', event.threadID, () => fs.unlinkSync(path), event.messageID);
...
module.exports.run = async function ({ api, event, args }) {
    if (args.length == 0 || !args) return api.sendMessage('» » pgl hou k ba song ka name likha yar😂💝🥵 empty!', event.threadID, event.messageID);
